...
class TapClient(LightCurvesDb):
    ...
    def postQuery(self, tap_params):
        ...
        if "/" in self.table and not (self.table.startswith('"') or self.table.startswith("'")):
    ...
    def _transfromCoo(self, condition):        
        if isinstance( condition[1], AbstractCoordinate):
            new_cond = [condition[0]]
            for i in range(1, len(condition)):
                new_cond.append(condition[i].getDegrees() * self.COO_UNIT_CONV )
            return new_cond
        return condition
