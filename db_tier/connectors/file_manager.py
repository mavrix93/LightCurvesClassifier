...
class FileManager(LightCurvesDb):
    ...
    def _createStarFromFITS(self, fits_name):
        ...
        star.name =  prim_hdu.get( self.FITS_NAME )        
        star.starClass = prim_hdu.get( self.FITS_CLASS )
        star.ra = RightAscension(ra, ra_unit)
        star.dec = Declination( dec, dec_unit)
    ...
    def _createLcFromFits(self, fits):
        ...
        for line in fits.data:
            try:
                ...
                raise InvalidFile("Light curve extension of fits couldn't be parsed\n%s" % line)
    ...
    def writeToFITS( self, file_name, star, clobber = True):
        ...
        prim_hdu.header[ self.FITS_RA ] = star.ra.degrees
        prim_hdu.header[ self.FITS_RA_UNIT ] = "deg"
        prim_hdu.header[ self.FITS_DEC ] = star.dec.degrees
