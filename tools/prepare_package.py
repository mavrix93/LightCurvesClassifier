...
def create_data_structure(examples=False):
    ...
    ----------
    examples : bool
        Example scripts are created if it is True
    ...
    to_create = [TO_THE_DATA_FOLDER, LC_FOLDER,
                 FILTERS_PATH, INPUTS_PATH, TUNING_LOGS, DB_FILE_PATH]
...
def make_examples():
    """
    Launch example scripts

    Returns
    -------
        None
    """
    os.system(os.path.join(settings.BASE_DIR,
                           'bin/prepare_query.py -o examples/query_folders.txt -p path -r "lpv;dpv" -p files_limit -r 10'))
    os.system(os.path.join(settings.BASE_DIR,
                           "bin/prepare_query.py -o examples/tuning_abbe_filter.txt  -p bins -r 10:150:10 "))
    os.system(os.path.join(settings.BASE_DIR,
                           """bin/prepare_query.py -o examples/tuning_color_index2.txt -p colors -r '["b_mag - r_mag"]' """))
    os.system(os.path.join(settings.BASE_DIR,
                           """bin/prepare_query.py -o examples/tuning_color_index.txt -p colors -r '["v_mag-i_mag","b_mag-v_mag"];["v_mag","i_mag"]' """))
    os.system(os.path.join(settings.BASE_DIR,
                           """bin/prepare_query.py -o examples/tuning_histvario_filter.txt -p hist_days_per_bin -r "97;80" -p vario_days_per_bin -r 9 -p vario_alphabet_size -r 16 -p hist_alphabet_size -r 7 """))
    os.system(os.path.join(settings.BASE_DIR,
                           """bin/prepare_query.py -o examples/query_ogle.txt -p starid -r 1:10 -p target -r lmc -p field_num -r 1"""))

    os.system(os.path.join(settings.BASE_DIR,
                           """bin/make_filter.py  -i examples/tuning_abbe_filter.txt -f AbbeValueFilter -s quasars -c stars -d GaussianNBDec -o examples/AbbeValue_quasar.filter -l examples/AbbeValue_quasar"""))
...
def move_example_data(loc=None):
    """
    Move example data light curves to data folders

    Parameters
    ----------
    loc : str
        Location of example light curves
    """
    root = settings.ROOT_DIR
    if not loc:
        loc = os.path.join(root, "src", "examples", "examples_data")

    copy_tree(os.path.join(loc, "light_curves"), os.path.join(
        settings.TO_THE_DATA_FOLDER, "light_curves"))
