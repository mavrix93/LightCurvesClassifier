...
class SAX(object):
    ...
    def to_letter_rep(self, x):
        ...
        """
        (paaX, indices) = to_PAA(normalize(x,self.eps),self.wordSize)
        self.scalingFactor = np.sqrt(len(x) / float(self.wordSize))
...
class OverlapSpecifiedIsNotSmallerThanWindowSize:
    pass